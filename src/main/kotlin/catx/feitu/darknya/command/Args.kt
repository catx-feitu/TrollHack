...
import catx.feitu.darknya.gui.GuiManager
import dev.luna5ama.trollhack.gui.hudgui.AbstractHudElement
import catx.feitu.darknya.manager.managers.UUIDManager
...
) : AbstractArg<catx.feitu.darknya.module.AbstractModule>(), AutoComplete by DynamicPrefixMatch(::allAlias) {
    override suspend fun convertToType(string: String?): catx.feitu.darknya.module.AbstractModule? {
...
class HudElementArg(
    ...
    override suspend fun convertToType(string: String?): AbstractHudElement? {
        return catx.feitu.darknya.gui.GuiManager.getHudElementOrNull(string)
    }

    private companion object {
        val allAlias by CachedValueN(5000L) {
            catx.feitu.darknya.gui.GuiManager.hudElements.asSequence()
                .flatMap {
                    sequence {
                        yield(it.internalName)
                        it.alias.forEach {
