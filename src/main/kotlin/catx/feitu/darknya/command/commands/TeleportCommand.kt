...
object TeleportCommand : ClientCommand(
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.teleport(x: Int, y: Int, z: Int) {
        teleport(x + 0.5, y.toDouble(), z + 0.5)
    }

    private fun catx.feitu.darknya.event.SafeClientEvent.teleport(x: Double, y: Double, z: Double) {
        player.setPosition(x, y, z)
        connection.sendPacket(CPacketPlayer.Position(x, y, z, player.onGround))
    }

    private fun catx.feitu.darknya.event.SafeClientEvent.teleport(x: Int, y: Int, z: Int, speed: Double) {
        teleport(x + 0.5, y.toDouble(), z + 0.5, speed)
    }

    private fun catx.feitu.darknya.event.SafeClientEvent.teleport(x: Double, y: Double, z: Double, speed: Double) {
        if (speed.isNaN()) {
            player.setPosition(x, y, z)
            connection.sendPacket(CPacketPlayer.Position(x, y, z, player.onGround))
        } else {
