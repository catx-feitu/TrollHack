...
object HotbarSwitchManager : Manager() {
    ...
    fun catx.feitu.darknya.event.SafeClientEvent.ghostSwitch(slot: Slot, block: () -> Unit) {
        ghostSwitch(Override.DEFAULT, slot, block)
    }

    fun catx.feitu.darknya.event.SafeClientEvent.ghostSwitch(slot: Int, block: () -> Unit) {
        ghostSwitch(Override.DEFAULT, slot, block)
    }

    fun catx.feitu.darknya.event.SafeClientEvent.ghostSwitch(override: Override, slot: Slot, block: () -> Unit) {
        synchronized(InventoryTaskManager) {
    ...
    fun catx.feitu.darknya.event.SafeClientEvent.ghostSwitch(override: Override, slot: Int, block: () -> Unit) {
        ghostSwitch(override, player.hotbarSlots[slot], block)
    }

    enum class BypassMode {
        NONE {
            override fun catx.feitu.darknya.event.SafeClientEvent.switch(targetSlot: Slot, block: () -> Unit) {
        ...
        abstract fun catx.feitu.darknya.event.SafeClientEvent.switch(targetSlot: Slot, block: () -> Unit)
    }
