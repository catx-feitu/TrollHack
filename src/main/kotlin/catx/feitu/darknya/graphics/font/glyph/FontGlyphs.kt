package catx.feitu.darknya.graphics.font.glyph
...
import catx.feitu.darknya.graphics.glMapNamedBufferRange
import catx.feitu.darknya.graphics.texture.BC4Compression
import catx.feitu.darknya.graphics.texture.Mipmaps
import catx.feitu.darknya.graphics.texture.RawImage
...
class FontGlyphs(val id: Int, private val font: Font, private val fallbackFont: Font, private val textureSize: Int) {
    ...
    init {
        ...
        val buffer = _root_ide_package_.catx.feitu.darknya.graphics.glMapNamedBufferRange(
            bufferID,
            0,
            cache.data.size.toLong(),
            GL_MAP_WRITE_BIT
        )
    ...
    private suspend fun loadGlyphChunkAsync(chunkID: Int): GlyphChunk {
        return coroutineScope {
            ...
            val deferredBuffer = onMainThread {
                val bufferID = glCreateBuffers()
                glNamedBufferStorage(bufferID, cache.data.size.toLong(), GL_MAP_WRITE_BIT)
                bufferID to _root_ide_package_.catx.feitu.darknya.graphics.glMapNamedBufferRange(
                    bufferID,
                    0,
                    cache.data.size.toLong(),
                    GL_MAP_WRITE_BIT
