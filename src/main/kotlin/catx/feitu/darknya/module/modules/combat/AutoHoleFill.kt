...
internal object AutoHoleFill : Module(
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.updatePosRender(holeInfos: List<IntermediateHoleInfo>) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.getPos(holeInfos: List<IntermediateHoleInfo>, checkRotation: Boolean): BlockPos? {
        val set = LongOpenHashSet()
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.calcPosSequence(holeInfos: List<IntermediateHoleInfo>): Sequence<Pair<EntityPlayer, IntermediateHoleInfo>> {
        return sequence {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.getRotationPos(holeInfos: List<IntermediateHoleInfo>): BlockPos? {
        return calcPosSequence(holeInfos)
            .minByOrNull { it.first.horizontalDist(it.second.center) }
            ?.second?.blockPos
    }

    private fun catx.feitu.darknya.event.SafeClientEvent.getHoleInfos(): List<IntermediateHoleInfo> {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.placeBlock(slot: Slot, pos: BlockPos) {
        val target = pos.down()
