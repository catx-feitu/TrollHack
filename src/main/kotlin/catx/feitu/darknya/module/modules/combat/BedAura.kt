...
import catx.feitu.darknya.graphics.mask.EnumFacingMask
import dev.luna5ama.trollhack.gui.hudgui.elements.client.Notification
import catx.feitu.darknya.manager.managers.CombatManager
...
internal object BedAura : Module(
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.refillBed(hotbarSlot: HotbarSlot) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.instantTiming(placeInfo: PlaceInfo, validDamage: Boolean) {
        if (validDamage) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.syncTiming(placeInfo: PlaceInfo, validDamage: Boolean) {
        if (validDamage) {
            if (timer.tick(getDelay(placeInfo, delay, slowDelay))) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.switchTiming(placeInfo: PlaceInfo, validDamage: Boolean) {
        if (validDamage) {
            if (switchPlacing) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.breakIfPlaced(placeInfo: PlaceInfo, delay: Int) {
        if (timer.tick(delay)) {
            if (isBedPlaced(placeInfo)) {
                breakBed(placeInfo)
            }
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.isBedPlaced(placeInfo: PlaceInfo): Boolean {
        return world.getBlock(placeInfo.bedPosFoot) == Blocks.BED
            || world.getBlock(placeInfo.bedPosHead) == Blocks.BED
    }

    private fun catx.feitu.darknya.event.SafeClientEvent.breakBed(placeInfo: PlaceInfo) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.placeBed(placeInfo: PlaceInfo
                                         ) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.update() {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.calcPlaceInfo(): PlaceInfo? {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.isValidBasePos(basePos: BlockPos): Boolean {
        return world.getBlockState(basePos).isSideSolid(world, basePos, EnumFacing.UP)
    }

    private fun catx.feitu.darknya.event.SafeClientEvent.isValidBedPos(ignoreNonFullBox: Boolean, calcInfo: CalcInfo): Boolean {
