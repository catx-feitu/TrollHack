...
internal object KillAura : Module(
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.runKillAura() {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.rotate(target: EntityLivingBase) {
        when (rotationMode) {
            RotationMode.SPOOF -> {
                sendPlayerPacket {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.canAttack(target: EntityLivingBase): Boolean {
        return when (mode) {
            Mode.COOLDOWN -> {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.attack(entity: Entity) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.startBlocking() {
        if (!blocking) {
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.stopBlocking() {
        if (blocking) {
