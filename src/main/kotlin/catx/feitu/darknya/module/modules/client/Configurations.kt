...
import catx.feitu.darknya.setting.ConfigManager
...
internal object Configurations : catx.feitu.darknya.module.AbstractModule(
    name = "Configurations",
    ...
    private fun updatePreset(setting: StringSetting, input: String, config: IConfig) {
        ...
        try {
            catx.feitu.darknya.setting.ConfigManager.save(config)
            setting.value = nameWithoutExtension
            catx.feitu.darknya.setting.ConfigManager.save(GenericConfig)
            catx.feitu.darknya.setting.ConfigManager.load(config)
    ...
    interface IConfigType : DisplayEnum {
        ...
        fun reload() {
            DefaultScope.launch(Dispatchers.IO) {
                var loaded = catx.feitu.darknya.setting.ConfigManager.load(GenericConfig)
                loaded = catx.feitu.darknya.setting.ConfigManager.load(config) || loaded
        ...
        fun save() {
            DefaultScope.launch(Dispatchers.IO) {
                var saved = catx.feitu.darknya.setting.ConfigManager.save(GenericConfig)
                saved = catx.feitu.darknya.setting.ConfigManager.save(config) || saved
