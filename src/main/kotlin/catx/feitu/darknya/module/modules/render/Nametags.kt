...
import catx.feitu.darknya.graphics.RenderUtils2D
import catx.feitu.darknya.graphics.RenderUtils3D
import catx.feitu.darknya.graphics.VAlign
import catx.feitu.darknya.graphics.color.ColorGradient
...
import catx.feitu.darknya.manager.managers.TotemPopManager
...
internal object Nametags : Module(
    ...
    private fun catx.feitu.darknya.event.SafeClientEvent.getContent(contentType: ContentType, entity: Entity) = when (contentType) {
        ContentType.NONE -> {
            null
        }
        ContentType.NAME -> {
    ...
    private class ItemGroupRenderInfo(event: catx.feitu.darknya.event.SafeClientEvent, itemGroup: ItemGroup) : IRenderInfo {
    ...
    private class XpOrbRenderInfo(event: catx.feitu.darknya.event.SafeClientEvent, private val entity: EntityXPOrb) : IRenderInfo {
    ...
    private class LivingRenderInfo(event: catx.feitu.darknya.event.SafeClientEvent, private val entity: EntityLivingBase) : IRenderInfo {
        override val distanceSq = event.player.distanceSqTo(entity).toFloat()
        private val textComponent = TextComponent()
